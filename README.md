...
|----------|----|--------|----
|textQuery|String|No|Restrict markets by any text associated with the market such as the Name, Event, Competition, etc. You can include a wildcard (*) character as long as it is not the first character.
|exchangeIds|JSON Array|No|**DEPRECATED**
|eventTypeIds|JSON Array|No|Restrict markets by event type associated with the market. (i.e., Football, Hockey, etc). Example: `["exampleType", "exampleType2"]`
|eventIds|JSON Array|No|Restrict markets by the event id associated with the market.  Example: `["exampleId1", "exampleId2"]`.
|competitionIds|JSON Array|No|Restrict markets by the competitions associated with the market. Example: `["exampleId1", "exampleId2"]`.
|marketIds|JSON Array|No|Restrict markets by the market id associated with the market. Example: `["exampleId1", "exampleId2"]`.
|venues|Boolean|No|Restrict markets by the market id associated with the market. Example: `["exampleId1", "exampleId2"]`.
|bspOnly|Boolean|No|Restrict to bsp markets only, if True or non-bsp markets if False. If not specified then returns both BSP and non-BSP markets
|turnInPlayEnabled|Boolean|No|Restrict to markets that will turn in play ifTrue or will not turn in play if false. If not specified, returns both
|inPlayOnly|Boolean|No|Restrict to markets that are currently in play if True or are not currently in play if false. If not specified, returns both.
|marketBettingTypes|Boolean|No|Restrict to markets that match the betting type of the market (i.e. Odds, Asian Handicap Singles, or Asian Handicap Doubles. Valid values: `ODDS`, `LINE`, `RANGE`, `ASIAN_HANDICAP_DOUBLE_LINE`, `ASIAN_HANDICAP_SINGLE_LINE`, `FIXED_ODDS`. Example: `["ODDS", "LINE"]`
|marketCountries|JSON Array|No|Restrict to markets that are in the specifiedcountry or countries. Example: `["GBR"]`
|marketTypeCodes|JSON Array|No|Restrict to markets that match the type of the market (i.e., MATCH_ODDS, HALF_TIME_SCORE) . You should use this instead of relying on the market name as the market type codes are the same in all locales. Example: `["MATCH_ODDS", "HALF_TIME_SCORE"]`
|marketStartTime|JSON Object(TimeRange)|No|Restrict to markets that I have one or more orders in these status. Valid values: `PENDING`, `EXECUTION_COMPLETE`, `EXECUTABLE`, `EXPIRED`. Example: `["PENDING", "EXECUTION_COMPLETE"]`
...
|Field name|Type|Required|Description
|----------|----|--------|----
|priceData|String|No|The basic price data you want to receive in the response. Valid values: `SP_AVAILABLE`, `SP_TRADED`, `EX_BEST_OFFERS`, `EX_ALL_OFFERS`, `EX_TRADED`.
|exBestOffersOverrides|JSON(ExBestOffersOverrides)|No|Options to alter the default representation of best offer prices Applicable to EX_BEST_OFFERS priceData selection
|virtualise|Boolean|No|Indicates if the returned prices should include virtual prices. Applicable to EX_BEST_OFFERS and EX_ALL_OFFERS priceData selections, default value is false.
|rolloverStakes|Boolean|No|Indicates if the volume returned at each price point should be the absolute value or a cumulative sum of volumes available at the price and all better prices. If unspecified defaults to false. Applicable to EX_BEST_OFFERS and EX_ALL_OFFERS price projections. Not supported as yet.
...
|Field name|Type|Required|Description
|----------|----|--------|----
|bestPricesDepth|Number|No|The maximum number of prices to return on each side for each runner. If unspecified defaults to 3. Maximum returned price depth returned is 10.
|rollupModel|String|No|The model to use when rolling up available sizes. If unspecified defaults to STAKE rollup model with rollupLimit of minimum stake in the specified currency. Valid values: `STAKE`, `PAYOUT`, `MANAGED_LIABILITY`, `NONE`.
|rollupLimit|Number|No|The volume limit to use when rolling up returned sizes. The exact definition of the limit depends on the rollupModel. If no limit is provided it will use minimum stake as default the value. Ignored if no rollup model is specified.
|rollupLiabilityThreshold|Number|No|Only applicable when rollupModel is MANAGED_LIABILITY. (rollupLiabilityFactor * rollupLimit) is the minimum liabilty the user is deemed to be comfortable with. After the rollupLiabilityThreshold price subsequent volumes will be rolled up to minimum value such that the liability >= the minimum liability.service level default (5). Not supported as yet.
|rollupLiabilityFactor|Number|No|Only applicable when rollupModel is MANAGED_LIABILITY. (rollupLiabilityFactor * rollupLimit) is the minimum liabilty the user is deemed to be comfortable with. After the rollupLiabilityThreshold price subsequent volumes will be rolled up to minimum value such that the liability >= the minimum liability.service level default (5). Not supported as yet.
...
|Field name|Type|Required|Description
|----------|----|--------|----
|orderType|String|Yes|Valid values: `LIMIT`, `LIMIT_ON_CLOSE`, `MARKET_ON_CLOSE`.
|selectionId|Long|Yes|The selection_id.
|handicap|double|No|The handicap associated with the runnerin case of Asian handicap markets, null otherwise.
|side|String|No|Valid values: `BACK`, `LAY`.
|limitOrder|JSON Object(LimitOrder)|No|A simple exchange bet for immediate execution. See example & description bellow.
|limitOnCloseOrder|JSON Object(LimitOnCloseOrder)|No|Bets are matched if, and only if, the returned starting price is better than a specified price. In the case of back bets, LOC bets are matched if the calculated starting price is greater than the specified price. In the case of lay bets, LOC bets are matched if the starting price is less than the specified price. If the specified limit is equal to the starting price, then it may be matched, partially matched, or may notbe matched at all, depending on how much is needed to balance all bets against each other (MOC, LOC and normal exchange bets). See example & description bellow.
|marketOnCloseOrder|JSON Object(MarketOnCloseOrder)|No|Bets remain unmatched until the market is reconciled. They are matched and settled at a price that is representative of the market at the point the market is turned in-play. The market is reconciled to find a starting price and MOC bets are settled at whatever starting price is returned. MOC bets are always matched and settled, unless a starting price is not available for the selection. Market on Close bets can only be placed before the starting price is determined. See example & description bellow.
|customerOrderRef|String|No|An optional reference customers can set to identify instructions.. No validation will be done on uniqueness and the string is limited  ters. If an empty string is provided it will be treated as null.
...
|Field name|Type|Required|Description
|----------|----|--------|----
|size|double|Yes|The size of the bet. **Please note**: For market type EACH_WAY. The total stake size x 2
|price|double|Yes|The limit price
|persistenceType|String|Yes|What to do with the order at turn-in-play. Valid values: `LAPSE`, `PERSIST`, `MARKET_ON_CLOSE`.
|timeInForce|String|No|The type of TimeInForce value to use. This value takes precedence over any PersistenceType value chosen. Valid value: FILL_OR_KILL
|minFillSize|Size|No|An optional field used if the TimeInForce attribute is populated.
|betTargetType|String|No|An optional field to allow betting to a targeted PAYOUT or BACKERS_PROFIT.
|betTargetSize|Size|No|An optional field which must bespecified if BetTargetType is specified for this order 
...
|Field name|Type|Required|Description
|----------|----|--------|----
|liability|double|Yes|The size of the bet.
|price|double|Yes|The limit price of the bet if LOC
...
|Field name|Type|Required|Description
|----------|----|--------|----
|betId|String|Yes|Unique identifier for the bet
|sizeReduction|double|No|If supplied then this is a partial cancel.  Should be set to 'null' if no size reduction is required.
...
|----------|----|--------|----
|betId|String|Yes|Unique identifier for the bet
|newPrice|double|Yes|The price to replace the bet at
...
|----------|----|--------|----
|betId|String|Yes|Unique identifier for the bet
|newPersistenceType|String|Yes|Valid values: `LAPSE`, `PERSIST`, `MARKET_ON_CLOSE`.
...
|Field name|Type|Required|Description
|----------|----|--------|----
|version|Long|No|A non-monotonically increasing number indicating market changes
